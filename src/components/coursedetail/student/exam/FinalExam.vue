      ...
      <el-collapse v-model="activeNames" @change="handleChange">
        <div class="exam-content" v-for="(item, index) in examInfoList" :key="index">
          <div v-if="item.fraction != 0">
            <div class="unitName">
              <span class="unitChapter">期末考试</span>&nbsp;
              <span class="unitEndTime">截止时间:{{dateFormate(new Date(item.endTime))}}</span>
              <!-- <router-link class="doUnitExam" :to="{path:'/studycourse/beginUnitExam',query:{id:item.id,name:item.examName}}"><el-button type="success" class="enterExam">进入考试</el-button></router-link> -->
              <el-button type="success" class="enterExam" :disabled="score[index] != null" @click="DoFinalExam(item.id,item.examName)">进入考试</el-button>
