      ...
      <el-collapse v-model="activeNames" @change="handleChange">
        <div class="exam-content" v-for="(item, index) in examInfoList" :key="index">
          <div v-if="item.fraction != 0">
            <div class="unitName">
              <span class="unitChapter">考试{{index+1}}</span>&nbsp;
              <span class="unitEndTime">截止时间:{{dateFormate(new Date(item.endTime))}}</span>
              <!-- <router-link class="doUnitExam" :to="{path:'/studycourse/beginUnitExam',query:{id:item.id,name:item.examName}}"><el-button type="success" class="enterExam" :disabled="score[index] != null">进入考试</el-button></router-link> -->
              <el-button type="success" class="enterExam" :disabled="score[index] != null" @click="DoUnitExam(item.id,item.examName)">进入考试</el-button>
              <span class="score" v-show="score[index] != null">考试成绩:{{score[index]}}</span>
            </div>
          
            <el-collapse-item class="info" :title="item.examName">
              <div class="info_show">
                <div class="info_item">
