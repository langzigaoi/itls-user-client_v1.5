...
export default {
  ...
  methods: {
      ...
      this.$refs['addForm'].validate((valid) => {
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            // console.log(params);
            this.$api.problem.multichoice.add(params).then((res) => {
              if (res.code == 200) {
      ...
      this.$refs['editForm'].validate((valid) => {
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            // console.log(params);
            this.$api.problem.multichoice.update(params).then((res) => {
              if (res.code == 200) {
                this.$message({ message: "操作成功", type: "success" });
                this.closeEditForm();
                this.flag = "2"
    ...
    handleRemove(data){
      ...
      this.$api.problem.multichoice.del(data.params)
          .then(data != null ? data.callback : "");
