...
<template>
  <div class="outside" style="padding: 10px">
    <el-row type="flex">
      <el-col :span="14">
        <el-tabs v-model="typeFlag" type="card" @tab-click="handleChangeTypeFlag" class="mlef-20">
      ...
      <el-col :span="10" align="center">
        <el-cascader style="width: 150px; margin-left: 10px" size="mini" placeholder="请选择范围" :options="allKnowledge"
          :props="{ checkStrictly: true }" @change="handleSearchChapter" :clearable="true"></el-cascader>

        <el-button style="margin-left: 5px" size="mini" type="primary" @click="chandgeAddVisible">新增</el-button>
      </el-col>
    </el-row>
    ...
    <!--考试列表 
      ...
      -->
    <com-table :data="pageResults" :columns="columns" :flag="'2'" @findPersonPage="findPersonPage"
      :showFlag="examList.showFlag" :disableFlag="examList.disableFlag" @handleRemove="handleRemove"
      @openSetupForm="changeSetupFormVisible"  @handlePreview="handlePreview" @handlePub="handlePub">
    </com-table>


    <el-dialog align="center" :visible.sync="addVisible" width="60%" style="" @close="closeAddForm">
          ...
          <el-row type="flex" justify="center">
            ...
            <el-col :span="18">
              <el-form-item label="标题" label-width="110px" prop="examName">
                <el-row type="flex">
                  <el-input style="width: 205px" size="mini" v-model="addForm.examName" placeholder="请输入内容"></el-input>
                </el-row>
              </el-form-item>

              <el-form-item label="类型" label-width="110px" prop="examTypeId">
                <el-row type="flex">
                  <el-select size="small" v-model="addForm.examTypeId" placeholder="请选择">
              ...
              <el-form-item label="范围" label-width="110px" prop="knowledgeId">
                <el-row type="flex">
                  <el-cascader ref="knowledge" :show-all-levels="false" size="mini" style="width: 205px"
                    v-model="addForm.knowledgeId" placeholder="请选择" :options="allKnowledge" :props="{
                      checkStrictly: true,
                      emitPath: false,
                      multiple: true,
              ...
              <el-form-item label="开始时间" label-width="110px" prop="startTime">
                <el-row type="flex">
                  <el-date-picker style="width: 205px" size="small" align="left" v-model="addForm.startTime"
              ...
              <el-form-item label="结束时间" label-width="110px" prop="endTime">
                <el-row type="flex">
                  <el-date-picker style="width: 205px" size="small" align="left" v-model="addForm.endTime"
              ...
              <el-form-item label="时长(分钟)" label-width="110px" prop="duration">
                <el-row type="flex">
                  <el-input style="width: 205px" size="mini" v-model="addForm.duration" placeholder="请输入内容"></el-input>
                </el-row>
              </el-form-item>
              <el-form-item label="成绩公布时间" label-width="110px" prop="resultPubTime">
                <el-row type="flex">
                  <el-date-picker style="width: 205px" size="small" align="left" v-model="addForm.resultPubTime"
                    type="datetime" placeholder="选择日期" :picker-options="pickerOptions">
                  </el-date-picker>
                </el-row>
              </el-form-item>
          ...
          <el-row>
            <div>
              <el-row type="flex" align="bottom" justify="end">
                ...
                <el-button type="primary" size="mini" @click="submitAdd">保存并离开</el-button>
              </el-row>
            </div>
          </el-row>
        </div>
    ...
    <el-dialog top="5vh" align="center" title="" :visible.sync="setupFormVisible" width="70%" style=""
      @close="closeSetupForm">
      <el-form ref="setupForm" :model="setupForm">
        <el-row text-align="left">
          <el-tabs v-model="active" @tab-click="handleTabClick">
            <el-tab-pane label="1.基本信息" name=1></el-tab-pane>
            <el-tab-pane label="2.题型设置" name=2></el-tab-pane>
            <el-tab-pane label="3.题目设置" name=3></el-tab-pane>
            <el-tab-pane label="4.双向细目" name=4></el-tab-pane>
          </el-tabs>
        </el-row>

        <div style="padding-top:20px">
          <!-- 1.基本信息表单-->
          <el-form v-if="active == 1" ref="infoForm" :model="infoForm" :rules="infoFormRules">
            <el-row type="flex">
              <el-col :span="3"></el-col>
              <el-col :span="10">
                <el-form-item label="标题" label-width="110px" prop="examName">
                  <el-row type="flex">
                    <el-input style="width: 205px" size="mini" v-model="infoForm.examName" placeholder="请输入内容">
                    </el-input>
                  </el-row>
                </el-form-item>
                <el-form-item label="类型" label-width="110px" prop="examTypeId">
              ...
              <el-col :span="11">
                ...
                <el-form-item label="时长(分钟)" label-width="110px" prop="duration">
                  <el-row type="flex">
                    <el-input style="width: 205px" size="mini" v-model="infoForm.duration" placeholder="请输入内容">
                    </el-input>
                  </el-row>
                </el-form-item>
          ...
          <!-- 2.题型设置表单-->
          <el-form v-if="active == 2" :model="itemListForm" ref="itemListForm" size="small" text-algin="center">
            <el-form-item>
              <div style="width:90%">
          ...
          <!-- 3.题目设置表单-->
          <el-form v-if="active == 3" ref="viewItemForm" size="small" text-algin="center">
            <el-form-item>
              <div style="width:90%">
                <view-item-table :data="viewItemForm" 
          ...
          <!-- 4.双向细目表单-->
          <el-form v-if="active == 4" :model="objectiveListForm" ref="objectiveListForm" size="small"
            text-algin="center">
            <el-form-item>
              <div style="width:90%">
                <objective-table :data="objectiveListForm" :allProblemType="allProblemType"
    ...
    <el-dialog top="5vh" align="center" width="70%" :visible.sync="previewProblemListVisible"
      ...
      <el-form :model="previewProblemListForm" ref="previewProblemListForm" size="small" text-algin="center">
        ...
        <el-form-item>
          <div style="width:80%">
            <modifiable-table :data="previewProblemListForm" :columnFlag="problemColumnFlag" :showOperation="false">
            </modifiable-table>
          </div>
        </el-form-item>
    ...
    <!--题目列表页面-第二层-->
    <el-dialog top="5vh" align="center" :visible.sync="problemListVisible" width="70%" @close="closeProblemListForm">
      <el-form :model="problemListForm" ref="problemListForm" size="small" text-algin="center">
        ...
        <el-form-item>
          <div style="width:80%">
            <modifiable-table :data="problemListForm" :allProblemType="allProblemType" :columnFlag="listColumnFlag"
              @open="openProblemInfo" @choose="changeAllProblemVisible" @randomChoose="handleRandomChoose" >
            </modifiable-table>
          </div>
        </el-form-item>
        ...
        <el-row>
          <div>
            <el-row type="flex" align="middle" justify="center">
              ...
              <el-button type="primary" size="mini" @click="submitProblemList">保存
              </el-button>
              <h5>
    ...
    <!--选题页面-第三层-->
    <el-dialog align="center" :visible.sync="allProblemVisible" width="80%">
      <el-form ref="allProblemForm" :model="allProblemForm">
        <div>
          <el-row>
            <el-col :span="6" align="left">
              <el-cascader ref="knowledge" size="mini" style="width: 100%;" :props="{
                checkStrictly: false,
                emitPath: false,
                multiple: true,
                collapse: true,
              }" :collapse-tags="true" :show-all-levels="false" placeholder="请选择第一级菜单中的选项" :options="allKnowledge"
                v-model="allProblemForm.knowledgeId" @change="handleSelectedKnowledge"></el-cascader>
          ...
          <el-row>
            <com-table ref="problemTable" :data="allProblemForm.tableData" :columns="problemColumns"
              :showOperation=false :flag="'1'" :showFlag="allProblem.showFlag" @findPage="findProblemPage"
              @handleSelections="handleAllProblemSelect" :selected="selected">
            </com-table>
          </el-row>
          ...
          <el-row>
            <div>
              <el-row type="flex" align="middle" justify="center">
                ...
                <el-button type="primary" size="mini" @click="submitAllProblem">提交
                </el-button>
                <h5>
                  (需选 {{ problemListForm.num }} 题,已选 {{ problemListForm.tableData.length }} 题)
                </h5>
    ...
    <!--单个题目展示弹框-->
    <el-dialog top="5vh" align="center" :visible.sync="viewProblemVisible" width="70%" style=""
      @close="closeViewProblemForm">
      <el-form ref="viewProblemForm" :model="viewProblemForm">
        <div>
          <el-row type="flex" justify="center">
            <el-col :span="14">

              <el-form-item label="知识点" label-width="80px" v-if="viewProblemForm.knowledgeName">
                <el-row type="flex">
                  <el-input onfocus="this.blur();" v-model="viewProblemForm.knowledgeName" auto-complete="off"
                    size="mini"></el-input>
                </el-row>
              </el-form-item>
              ...
              <!--选择题特有字段-->
              <el-form-item v-if="viewProblemForm.title" label="题干" label-width="80px" prop="title">
                <el-row type="flex">
                  <el-input onfocus="this.blur();" style="textarea" :rows="4" size="mini" type="textarea"
                    v-model="viewProblemForm.title" placeholder="请输入内容"></el-input>
                </el-row>
              </el-form-item>
              <el-form-item label="选项" label-width="80px" prop="options" v-if="viewProblemForm.options">
                <el-row type="flex">
                  <el-input onfocus="this.blur();" type="textarea" :rows="4" v-model="viewProblemForm.options"
                    auto-complete="off" size="mini"></el-input>
                </el-row>
              </el-form-item>
              <el-form-item label="答案" label-width="80px" prop="answer" v-if="viewProblemForm.answer">
                <el-row type="flex">
                  <el-input onfocus="this.blur();" type="textarea" :rows="4" v-model="viewProblemForm.answer"
                    auto-complete="off" size="mini"></el-input>
                </el-row>
              </el-form-item>
              <el-form-item label="解析" label-width="80px" prop="analysis" v-if="viewProblemForm.analysis">
                <el-row type="flex">
                  <el-input onfocus="this.blur();" type="textarea" :rows="4" v-model="viewProblemForm.analysis"
                    auto-complete="off" size="mini"></el-input>
                </el-row>
              </el-form-item>
              ...
              <!--编程题特有字段-->
              <el-form-item label="名称" label-width="80px" prop="problemName" v-if="viewProblemForm.problemName">
                <el-row type="flex">
                  <el-input onfocus="this.blur();" style="textarea" size="mini" v-model="viewProblemForm.problemName"
              ...
              <el-form-item label="题干" label-width="80px" prop="problemDescription"
                ...
                <el-row type="flex">
                  <el-input onfocus="this.blur();" style="textarea" size="mini" type="textarea" :rows="4"
                    v-model="viewProblemForm.problemDescription" placeholder="请输入内容"></el-input>
                </el-row>
              </el-form-item>
              <el-form-item label="输入格式" label-width="80px" prop="problemInputFormat"
                ...
                <el-row type="flex">
                  <el-input onfocus="this.blur();" style="textarea" size="mini" type="textarea" :rows="4"
                    v-model="viewProblemForm.problemInputFormat" placeholder="请输入内容"></el-input>
                </el-row>
              </el-form-item>
              <el-form-item label="输入样例" label-width="80px" prop="problemSampleInput"
                ...
                <el-row type="flex">
                  <el-input onfocus="this.blur();" style="textarea" size="mini" type="textarea" :rows="4"
                    v-model="viewProblemForm.problemSampleInput" placeholder="请输入内容"></el-input>
                </el-row>
              </el-form-item>
              <el-form-item label="输出样例" label-width="80px" prop="problemSampleOutput"
                ...
                <el-row type="flex">
                  <el-input onfocus="this.blur();" style="textarea" size="mini" type="textarea" :rows="4"
                    v-model="viewProblemForm.problemSampleOutput" placeholder="请输入内容"></el-input>
                </el-row>
              </el-form-item>

              <el-form-item label="提示" label-width="80px" prop="problemHint" v-if="viewProblemForm.problemHint">
                <el-row type="flex">
                  <el-input onfocus="this.blur();" style="textarea" size="mini" type="textarea" :rows="4"
                    v-model="viewProblemForm.problemHint" placeholder="请输入内容"></el-input>
                </el-row>
              </el-form-item>
              ...
              <el-row>
                <el-col :span="12">
                  <el-form-item label="时间限制" label-width="80px" prop="problemTimeLimit"
                    ...
                    <el-row type="flex">
                      <el-input onfocus="this.blur();" style="width:65%; " size="mini"
                        v-model="viewProblemForm.problemTimeLimit" placeholder="请输入内容"></el-input>
                    </el-row>
                  </el-form-item>
                </el-col>
                <el-col :span="12">
                  <el-form-item label="内存限制" label-width="80px" prop="problemMemoryLimit"
                    ...
                    <el-row type="flex">
                      <el-input onfocus="this.blur();" style="width:65%; " size="mini"
                        v-model="viewProblemForm.problemMemoryLimit" placeholder="请输入内容"></el-input>
                    </el-row>
                  </el-form-item>
                </el-col>
              </el-row>
              ...
              <el-form-item label="题目来源" label-width="80px" prop="markup" v-if="viewProblemForm.markup">
                <el-row type="flex">
                  <el-input onfocus="this.blur();" v-model="viewProblemForm.markup" auto-complete="off" size="mini">
                  </el-input>
                </el-row>
              </el-form-item>
              <el-row>
                <el-col :span="12">
                  <el-form-item label="难易度" label-width="80px" prop="difficulty" align="left"
                    ...
                    <el-row type="flex">
                      <el-input v-model="viewProblemForm.difficulty" auto-complete="off" size="mini"></el-input>
                    </el-row>
                  </el-form-item>
                </el-col>
                <el-col :span="12">
                  <el-form-item label="区分度" label-width="80px" prop="discrimination" align="left"
                    ...
                    <el-row type="flex">
                      <el-input v-model="viewProblemForm.discrimination" auto-complete="off" size="mini"></el-input>
                    </el-row>
                  </el-form-item>
                </el-col>
              </el-row>
              <el-row>
                <div>
                  <el-row type="flex" align="bottom" justify="center">
                    <el-button size="mini" @click="closeViewProblemForm">返回
                    </el-button>
                  </el-row>
                </div>
              </el-row>
...
import ViewItemTable from './ViewItemTable.vue';
import ObjectiveTable from './ObjectiveTable.vue';
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { prop: "list", label: "范围", minWidth: 250, align: "center",  overflow:"true" },
        {
          prop: "startTime",
          label: "开始时间",
          minWidth: 150,
  ...
  methods: {
    ...
    handleTabClick() {
      ...
      this.findData(this.active);

    },
    findData(active){
      console.log(this.viewItemForm);
      if(active == 1){
        let examId = this.setupForm.id;
        this.$api.exam.examSetup.findById({examId: examId}).then((res) =>{
          // console.log(res);
          this.infoForm = JSON.parse(JSON.stringify(res.data))
          // 所选考试范围回显
          if (this.infoForm.chapterOfList != null && this.infoForm.chapterOfList != undefined 
          && this.infoForm.chapterOfList.length > 0) {
            let chapterList = [];
            for (let index = 0; index < this.infoForm.chapterOfList.length; index++) {
              chapterList.push(parseInt(this.infoForm.chapterOfList[index].chapterId));
            }
        ...
        console.log(this.itemColumnFlag);
        console.log(this.typeColumnFlag);
        ...
        this.$api.exam.examItem.findList(this.pageRequest)
          .then((res) => {
            ...
            if (res.data != null) {
              ...
              //处理章节查询参数
              this.knowledgeOption = this.infoForm.chapterOfList;
              let knowledgeString = "";
    ...
    changeSetupFormVisible(row) {
      console.log(row);
      this.setupForm = JSON.parse(JSON.stringify(row));
      this.handleTabClick();
    ...
    findProblemPage(data) {
      console.log(this.problemListForm);
      let pageRequest = {pageNum: 1, pageSize: 10 }
      //按题型选择相应方法查询
      if (data !== null) {
      ...
      if (lable == "编程题") {
        this.$api.problem.program.findByKid(pageRequest)
          .then((res) => {
            if (res.data !== null) {
      ...
      // return this.allProblemForm.tableData
    ...
    handleRandomChoose(){
      ...
      let label = this.problemListForm.name;

      this.$api.problem.selectProblem.selectProblemRandom
        ({ courseId: this.$store.state.course.courseId,
           problemTypeId: this.problemTypeLableToId(label),
