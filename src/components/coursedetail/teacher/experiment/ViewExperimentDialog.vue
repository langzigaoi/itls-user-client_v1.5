<template>
  <div>
    <br>
    <el-dialog :visible.sync="dialogVisible" width=860px @close="reset">
      <div align="center">
        <h2 style="color: black;">实验报告</h2>
        <!-- <router-link :to="{name:'GetPdf',query:{'id':this.$route.query.id}}" append>生成pdf</router-link>&nbsp;&nbsp;&nbsp; -->
        <el-button size="small" type="info" class="getword" @click="getWord()">下载实验报告</el-button>
        <div class="report" style="width: 800px;">
          ...
          <div class="alltr">
            ...
            <div class="alltd" style="width: 41%;">{{this.dateTimeFormate(new Date(this.experiment.startTime))}}</div>
            <div class="allth">截止时间</div>
            <div class="alltd" style="width: 41%;border-right: none;">{{this.dateTimeFormate(new
              Date(this.experiment.endTime))}}</div>
          </div>
          <div class="alltr">
            <div class="allth" style="line-height: 21px;">成绩公布时间</div>
            <div class="alltd" style="width: 41%;">{{this.dateTimeFormate(new Date(this.experiment.pubTime))}}</div>
...
<script>
  ...
  export default {
    ...
    methods: {
      getExperimentInfo(index) {
        this.experimentId = index
        this.$api.experiment.experiment.getExperimentInfo({ id: index }).then((res) => {
          this.experiment = res.data;
        })
      },
      getWord() {
        let url = baseUrl + "/course/experimentsetup/getPdf?id=" + this.experimentId;
      ...
      reset() {
        this.experiment = this.experimentNull;
      }
    },
  }
</script>
