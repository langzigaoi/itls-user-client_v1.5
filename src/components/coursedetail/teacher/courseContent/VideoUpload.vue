<template>
    <div>
        <el-dialog title="Video上传" top="3vh" :visible.sync="dialogVisible" width="40%" :before-close="handleClose">
            <!-- {{global}} -->
            <div >
    ...
    methods:{
     ...
     // 将修改的东西提交
      submit(){
        ...
        //   console.log(pptid);
            let videoId = [];
              ...
              if(item.id) //有id代表是原来上传的文件
                ...
                videoId.push(item.response.data.id)
          })
          
          let videoid = videoId.toString();
          console.log(videoid);
         //处理video的id
        ...
        //     this.Msg(res.msg);
        //   })
            this.$api.course.section.updatevideoid({videoId:videoid,sectionid:this.sectionId}).then((res) => {
            this.changedialogVisible();
            this.$emit('getAllweeks');
            this.Msg(res.msg);
          })
